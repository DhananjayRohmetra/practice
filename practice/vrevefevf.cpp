...
{
    int n,no;
    cout<<"enter the total number of jobs"<<endl;
    cin>>n;
    int vi[n]={0};
    int bt[n],mr[n];
    int q,w;
    vector <pair <int,int> > b;
    cout<<"enter the burst time and memory required for each process"<<endl;
    for(int i=0;i<n;i++)
    {
        cin>>q>>w;
        b.push_back(make_pair(w,q));
    }
    sort(b.begin(),b.end());
    for(int i=0;i<n;i++)
    {
    ...
    cin>>m;
    int me[m];
    cout<<"enter size of each memory block available "<<endl;
    for(int i=0;i<m;i++)
    {
        cin>>me[i];
    }
    ...
    while(no<n)
    {
       for(int i=0;i<n;i++)
       {
           for(int j=0;j<m;j++)
           {
               if(v[j].second>=mr[i]&&vi[i]!=1)
               {
                   wt[i]=v[j].first;
                   intf[i]=v[j].second-mr[i];
                   vi[i]=1;
                   v[j].first+=bt[i];
                   sort(v.begin(),v.end());
                   no++;
    ...
    for(int i=0;i<n;i++)
    {
        cout<<"waiting time of "<<i+1<<" process is "<<wt[i]<<"and int. frag is "<<intf[i]<<endl;
    }
    return 0;
}
